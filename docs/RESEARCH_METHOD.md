...
2. **Thiết kế artefact**: mô hình dữ liệu và provenance ([DATA_DICTIONARY.md](DATA_DICTIONARY.md)); pipeline bất đồng bộ ([ARCHITECTURE.md](ARCHITECTURE.md)); schema trích xuất JSON ràng buộc với confidence từng trường; lựa chọn model có căn cứ benchmark công khai ([research/02-model-selection.md](research/02-model-selection.md)). Mỗi thay đổi kiến trúc quan trọng được ghi trong [nhật ký quyết định](DECISIONS.md).
...
4. **Thí nghiệm đo lường**: trên evaluation dataset synthetic (bộ `samples/` sinh bằng script, mở rộng thành `eval/dataset-v1/`) và — khi được phép — văn bản thật của phường. Mỗi thí nghiệm có ID trong [nhật ký thí nghiệm](experiments/README.md).
...
## 4. Evaluation dataset có version
...
Các phương án được chạy trên cùng một phiên bản dataset. Model lớn hơn chỉ được đưa vào so sánh khi có mục tiêu và giới hạn phần cứng rõ ràng.

## 6. Kiểm soát tính đúng đắn của quá trình nghiên cứu

- Mọi con số hiệu năng/chất lượng chỉ được công bố khi có trong [nhật ký thí nghiệm](experiments/README.md) hoặc tài liệu kết quả kèm dữ liệu tái lập.
- Toàn bộ kiểm thử hiện có phải đạt khi mở rộng hệ thống (xem [chiến lược kiểm thử](TEST_STRATEGY.md)).
