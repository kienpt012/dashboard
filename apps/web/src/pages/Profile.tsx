...
export default function Profile(){
  ...
  const requirements=[
    {label:'Ít nhất 8 ký tự',met:passwords.newPassword.length>=8},
  ...
  async function submit(event:FormEvent){
    ...
    try{
      ...
      setSuccess('Mật khẩu đã được thay đổi. Phiên làm việc hiện tại đã được gia hạn an toàn.');
    }catch(reason){
    ...
    <div className="profile-layout">
      ...
      <section className="panel security-card" aria-labelledby="security-heading">
        ...
        <form className="profile-password-form" onSubmit={submit} noValidate>
          ...
          <div className="profile-password-input">
            <LockKeyhole/>
            <input id="new-password" required minLength={8} maxLength={128} type={visible.newPassword?'text':'password'} value={passwords.newPassword} onChange={event=>update('newPassword',event.target.value)} autoComplete="new-password" placeholder="Tạo mật khẩu mới an toàn" aria-describedby="password-requirements"/>
